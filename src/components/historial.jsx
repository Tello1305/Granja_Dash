...
export function Historial() {
    ...
    useEffect(() => {
        ...
        setIsLoading(true);
        axios.get(`${RUTAJAVA}/api/historial`, { headers: { Authorization: `Bearer ${auth.token}` } })
    ...
    useEffect(() => {
        ...
        const filtrosEstanActivos = filtros.fechaDesde || filtros.fechaHasta || filtros.temas.length > 0 || filtros.busqueda;
            ...
            const timer = setTimeout(() => {
                const fetchActividadesFiltradas = async () => {
                    try {
                        ...
                        const response = await axios.get(`${RUTAJAVA}/api/historial/bitacora`, {
    ...
    }, [auth?.token, filtros]); // La dependencia clave es 'filtros'
    ...
    const handleCheckboxChange = (e) => {
        ...
        setFiltros(prev => {
            const nuevosTemas = checked ? [...prev.temas, value] : prev.temas.filter(tema => tema !== value);
    ...
    const mostrarVistaDetallada = filtros.fechaDesde || filtros.fechaHasta || filtros.temas.length > 0 || filtros.busqueda;
            ...
            <div className="card mb-4">
                <div className="card-body">
                    ...
                    <div className="row g-3 mb-3">
                        <div className="col-md-3"><label>Desde</label><input type="date" name="fechaDesde" className="form-control" value={filtros.fechaDesde} onChange={handleInputChange} /></div>
                        <div className="col-md-3"><label>Hasta</label><input type="date" name="fechaHasta" className="form-control" value={filtros.fechaHasta} onChange={handleInputChange} /></div>
                        <div className="col-md-6"><label>Palabra Clave</label><input type="text" name="busqueda" className="form-control" placeholder="Buscar..." value={filtros.busqueda} onChange={handleInputChange} /></div>
                    </div>
                    ...
                    {actividades.length > 0 ? actividades.map(actividad => (
                        <div key={actividad.id_actividad} className="card mb-2">
                            <div className="card-body py-2">
                                ...
                                <small className="text-muted">{new Date(actividad.fechaHora).toLocaleString('es-PE')}</small>
                            </div>
                        </div>
                ...
                <>
                    ...
                    {resumenes.map(reporte => (
                        <div key={reporte.id_resumen} className="card mb-3">
                            <div className="card-header bg-light"><strong>Periodo: {reporte.periodo}</strong></div>
                            <div className="card-body">
